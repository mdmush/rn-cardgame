...
const Card = ({testID, value, isFlipped, onClick, isDisabled, isInactive}) => {
  ...
  return (
    <Pressable
      ...
      testID={testID}>
      <Animated.View
        className="absolute top-0 left-0 right-0 bottom-0 items-center justify-center rounded-[8px] bg-[#f97316]"
        style={[styles.cardContainer, frontAnimatedStyle]}>
        <Text className="text-[20px] font-bold text-white">{value}</Text>
      </Animated.View>

      <Animated.View
        className="absolute top-0 left-0 right-0 bottom-0 items-center justify-center rounded-[8px] bg-[#fcba03]"
